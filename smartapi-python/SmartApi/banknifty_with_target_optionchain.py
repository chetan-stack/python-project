...
from SmartApi import SmartConnect, get_strick_price_historicaldata  # or from SmartApi.smartConnect import SmartConnect
import time
...
def checkcandlestickpattern(df):
    ...
    bot_message = f'5 min signals for Banknifty {signals} and the time is {datetime.datetime.now()}'
    if len(signals) > 0:
        sendAlert(bot_message)
                ...
                if not df.empty:
                    if getorderBook():  # (check if BANKNIFTY order is not placed)
                        if close_pre > sup_pre and close_cl < sup_cl:
                            if getoptionchain.getparams('BANKNIFTY',df.close.values[-1],'pe') and get_strick_price_historicaldata.gethistoricalldata(pe_symbol['symbol'],pe_symbol['token']):
                                print('buy 1')
                            ...
                            if getoptionchain.getparams('BANKNIFTY',df.close.values[-1],'ce') and get_strick_price_historicaldata.gethistoricalldata(ce_symbol['symbol'],ce_symbol['token']):
                                print('buy 2')
                        ...
                        elif ('pe' in buytradedBANKNIFTY) and get_strick_price_historicaldata.checkexitconditio(pe_symbol['symbol'],pe_symbol['token']):
                        ...
                        elif ('ce' in buytradedBANKNIFTY) and get_strick_price_historicaldata.checkexitconditio(ce_symbol['symbol'],ce_symbol['token']):
...
def stetergycheck1min(symbol):
    try:
        ...
        tv = TvDatafeed(username, password)
        data = tv.get_hist(symbol='BANKNIFTY', exchange='NSE', interval=Interval.in_1_minute, n_bars=2000)
        #print(data)
        if data is not None:
            df = pd.DataFrame(
                data,
